...
const socket = socketIO.connect(Config.link, { transports: ["websocket"] });
const Messages = () => {
    ...
    async function fetchGroup(defaultGroupId = null, defaultisGroup = null) {
        function checkToken() {
            ...
            fetch(Config.link + "chat/checkToken", {
                headers: {
                    Authorization: `${token}`
                }
        ...
        const token = localStorage.getItem('token');
        const response = await fetch(Config.link + "chat/group", {
            headers: {
                Authorization: `${token}`
            }
        });
    ...
    const handleFileUpload = async () => {
        ...
        try {
            ...
            const response = await axios.post(Config.link + "user/upload", formData, {
                headers: {
    ...
    const handleMessageAction = (key, message) => {
        if (key === "copy") {
            ...
            fetch(Config.link + `chat/message/delete/${message._id}`, {
                method: "DELETE",
                headers: {
    ...
    const handleUpdateChat = () => {
        ...
        fetch(Config.link + `chat/message/update/${updateMessageId}`, {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json"
            },
